...
import { Box, TextField, Typography, CircularProgress, Grid, Card, CardMedia, CardContent } from '@mui/material';
    ...
    <Box sx={{ padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      {results.length > 0 && (
        <Box 
          sx={{ 
            width: '100%', 
            maxHeight: '80vh', // Set a maximum height for the results container
          ...
          <Grid container spacing={2}>
            {results.map((receta) => (
              <Grid item xs={12} key={receta.id}>
                ...
                >
                  <Link to={`/receta/${receta.id}`} style={{ textDecoration: 'none', width: '100%' }}>
                    <Card 
                      sx={{ 
                        display: 'flex', 
                        overflow: 'hidden',
